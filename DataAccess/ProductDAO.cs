    ...
    public class ProductDAO
    {
            ...
            while (dbDataReader.Read())
            {
                ProductObject product = new ProductObject(
                   ...
                   dbDataReader["ProductName"].ToString(),
                    Convert.ToInt32(dbDataReader["CategoryId"]),
                   dbDataReader["Weight"].ToString(),
                   SqlMoney.Parse(dbDataReader["UnitPrice"].ToString()),
                   Convert.ToInt32(dbDataReader["UnitsInStock"].ToString()));
                if (products == null)
                {
        ...
        public void UpdateProduct(ProductObject product)
        {
            ...
            command.CommandText = "UPDATE Product " +
                "SET ProductName = @productName , CategoryId = @categoryId, [Weight] = @weight, UnitPrice = @unitPrice, UnitsInStock = @unitsInStock " +
                "WHERE ProductId = @productId";
